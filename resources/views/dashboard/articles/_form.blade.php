...
<div class="space-y-6">
    <div>
        <x-input-label for="slug" :value="__('Slug (optional — auto-generated from the Arabic title if left blank)')" />
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            <x-input-label for="body_ar" :value="__('Body (Arabic) — HTML allowed: p, h2-h4, ul/ol/li, strong, em, a, img, blockquote. Everything else is stripped on save.')" />
            <textarea id="body_ar" name="body[ar]" dir="rtl" rows="10" class="mt-1 block w-full font-mono text-sm border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('body.ar', $isEdit ? $article->getTranslation('body', 'ar') : '') }}</textarea>
            <x-input-error :messages="$errors->get('body.ar')" class="mt-2" />
        </div>
        <div>
            <x-input-label for="body_en" :value="__('Body (English) — HTML allowed: p, h2-h4, ul/ol/li, strong, em, a, img, blockquote. Everything else is stripped on save.')" />
            <textarea id="body_en" name="body[en]" rows="10" class="mt-1 block w-full font-mono text-sm border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('body.en', $isEdit ? $article->getTranslation('body', 'en') : '') }}</textarea>
