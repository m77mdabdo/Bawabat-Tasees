    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('dashboard.countries.heading') }}
            </h2>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($countries as $country)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $country->is_active ? __('dashboard.common.yes') : __('dashboard.common.no') }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-end text-sm space-x-2 rtl:space-x-reverse">
                                    <a href="{{ route('dashboard.countries.edit', $country) }}" class="text-indigo-600 hover:text-indigo-900">{{ __('dashboard.common.edit') }}</a>
                                    <form action="{{ route('dashboard.countries.destroy', $country) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('dashboard.countries.confirm_delete') }}');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">{{ __('dashboard.countries.empty') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
