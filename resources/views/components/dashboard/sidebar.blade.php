...
<aside
    ...
    class="fixed inset-y-0 right-0 z-40 flex w-72 shrink-0 flex-col bg-dark-green transition-transform duration-200 ease-in-out lg:static lg:translate-x-0"
>
    <div class="flex h-20 shrink-0 items-center border-b border-white/10 px-6">
        <a href="{{ route('dashboard') }}">
            <img src="{{ asset('images/brand/logo-full-white-256.png') }}" alt="{{ config('app.name') }}" class="h-9 w-auto">
        </a>
    </div>
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto px-4 py-6">
        ...
        <x-dashboard.nav-group :label="__('المحتوى')" :active="$contentActive">
            <x-slot name="icon">
                <svg class="h-5 w-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
        ...
        <x-dashboard.nav-group :label="__('التسويق')" :active="$marketingActive">
            <x-slot name="icon">
                <svg class="h-5 w-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
